      ...
      <div className="rounded-2xl shadow-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-white p-6">
        ...
        <div className="mb-4 text-sm space-y-1">
          ...
          <div>
            ...
            <span className={state.liquorLiability ? 'text-green-600' : 'text-gray-500'}>
              {state.liquorLiability ? 'Yes' : 'No'}
            </span>
          </div>
        </div>
        <div className="border-t pt-3 mt-3 text-sm space-y-1">
          <div>
            <span className="font-semibold">Base Premium:</span>{' '}
            <span className="text-blue-700">${state.basePremium.toFixed(2)}</span>
          </div>
          <div>
            <span className="font-semibold">Liability Premium:</span>{' '}
            <span className="text-blue-700">${state.liabilityPremium.toFixed(2)}</span>
          </div>
          <div>
            <span className="font-semibold">Liquor Premium:</span>{' '}
            <span className="text-blue-700">${state.liquorLiabilityPremium.toFixed(2)}</span>
          </div>
